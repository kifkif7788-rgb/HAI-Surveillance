...
import { useActivityLogs, type ActivityLog } from "@/lib/activity-store";
    ...
    <div className="card-soft p-5">
      ...
      <div className="mt-5">
        <button
          ...
          className="btn-soft bg-sky text-sky-foreground gap-2 px-4 text-sm">
          📋 {showLog ? "ซ่อน" : "ดู"} ประวัติการเข้าใช้งาน
        </button>
...
function ActivityLogSection() {
  ...
  const [filterAction, setFilterAction] = useState<"" | "login" | "logout">("");

  const filtered = logs.filter((l) => {
    if (filterUser && !l.username.includes(filterUser) && !l.name.includes(filterUser)) return false;
  ...
  return (
    <div className="mt-4 card-soft p-5">
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div className="font-bold text-primary">📋 ประวัติการเข้าใช้งาน ({logs.length} รายการ)</div>
        <button onClick={refresh} className="btn-soft bg-muted text-foreground px-3 text-xs">🔄 รีเฟรช</button>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <input
          ...
          className="px-3 py-1.5 rounded-xl border border-border bg-white text-sm focus:outline-none focus:ring-2 focus:ring-ring w-44"
        />
        <select
          value={filterAction}
          onChange={(e) => setFilterAction(e.target.value as "" | "login" | "logout")}
      ...
      {loading ? (
        <div className="text-sm text-muted-foreground text-center py-6">กำลังโหลด...</div>
      ) : filtered.length === 0 ? (
        <div className="text-sm text-muted-foreground text-center py-6">
          {logs.length === 0 ? "ยังไม่มีประวัติ (ต้องรัน schema.sql เพื่อสร้างตาราง activity_log ก่อน)" : "ไม่มีรายการตามตัวกรอง"}
        </div>
      ) : (
        <div className="overflow-x-auto max-h-96 overflow-y-auto">
          <table className="w-full text-sm">
            <thead className="sticky top-0">
              <tr className="text-left text-muted-foreground bg-sky/20">
                <th className="p-2.5 rounded-l-xl whitespace-nowrap">วันเวลา</th>
                <th className="p-2.5 whitespace-nowrap">ผู้ใช้</th>
                <th className="p-2.5 whitespace-nowrap">ชื่อ</th>
                <th className="p-2.5 whitespace-nowrap">แผนก / หอผู้ป่วย</th>
                <th className="p-2.5 rounded-r-xl whitespace-nowrap">การกระทำ</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((l) => (
                <tr key={l.id} className="border-b border-border/40 hover:bg-sky/5">
                  <td className="p-2.5 text-xs text-muted-foreground whitespace-nowrap">{fmt(l.timestamp)}</td>
                  <td className="p-2.5 font-mono text-xs">{l.username}</td>
                  <td className="p-2.5 font-medium text-foreground">{l.name}</td>
                  <td className="p-2.5 text-muted-foreground">{l.role || "—"}</td>
                  <td className="p-2.5">
                    <span className={cn(
                      "inline-block px-2 py-0.5 rounded-full text-xs font-semibold",
                      l.action === "login" ? "bg-mint/40 text-mint-foreground" : "bg-muted text-muted-foreground",
                    )}>
                      {l.action === "login" ? "🔓 เข้าใช้งาน" : "🔒 ออกจากระบบ"}
                    </span>
                  </td>
                </tr>
              ))}
