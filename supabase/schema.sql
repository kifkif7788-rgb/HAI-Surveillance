...
create table if not exists surveys            ( id text primary key, data jsonb not null, updated_at timestamptz default now() );
create table if not exists interview_sessions ( id text primary key, data jsonb not null, updated_at timestamptz default now() );
...
declare t text;
begin
  foreach t in array array['records','users','monthly','or_monthly','departments','wards','kpi_manual','activity_log','surveys','interview_sessions'] loop
